...
local _find_item = function(v, k, text)
    return k:lower():find(text) and not v.has_flag('hidden') and v or nil
end
local _find_fluid = function(v, k, text)
    return k:lower():find(text) and v or nil
end
...
local function get_or_create_used_for_flow(player, destroy)
    ...
    elseif not flow then
        ...
        main_table.visible = choose.elem_value or false
...
local function build_search_table(items_table, items)
    ...
    items_table.parent.parent.parent.visible = #items_table.children > 0 or false
    --BUILD IF ONLY 1
end

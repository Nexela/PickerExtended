...
local function revive_belts(event)
    ...
    if
        event.created_entity.name == "entity-ghost" and match_to_revive[event.created_entity.ghost_type] and lib.is_named_bp(player.cursor_stack, "Belt Brush") and
            not pdata._belt_brush_shift
     then
...
-- Allow Upgrades: When a blueprint is placed check to see if it is a beltbrush bp and if it is destroy matched ghosts underneath.
local function placed_blueprint(event)
    local player, pdata = Player.get(event.player_index)
    local stack = lib.stack_name(player.cursor_stack, "blueprint", true)
    if stack and lib.is_named_bp(stack, "Belt Brush") then
        pdata._belt_brush_shift = event.shift_build
        if event.built_by_moving == false or (pdata.last_ghost_check or 0) <= event.tick - 2 then
...
Gui.on_click("^beltbrush_btn_", function(event)
    local value
    if event.element.name:find("_up") then
        value = 1
    elseif event.element.name:find("_dn") then
        value = -1
    elseif event.element.name:find("_reset") then
        value = -99999999999
    end
        increase_decrease_reprogrammer(event, value)
end)

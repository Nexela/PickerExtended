...
local function on_entity_settings_pasted(event)
    ...
    local source = event.source and event.source.supports_direction and (event.source.fluidbox and #event.source.fluidbox > 0) and
        ...
        event.source.prototype.fast_replaceable_group == "assembling-machine"
    local destination = event.destination and event.destination.supports_direction and (event.destination.fluidbox and #event.destination.fluidbox > 0) and
